...
# Azure Speech — キー（api-key モード）か リソース ID（Entra モード）の どちらかを設定
AZURE_SPEECH_REGION=eastus2
# api-key モード: リソースキーを設定（サーバー側で短時間トークンに交換。ブラウザには渡らない）
AZURE_SPEECH_KEY=
# Entra モード: 代わりに ARM リソース ID を設定
AZURE_SPEECH_RESOURCE_ID=/subscriptions/<sub-id>/resourceGroups/<rg>/providers/Microsoft.CognitiveServices/accounts/<name>
# 任意: 認証モードを固定（api_key | entra）。既定は自動判定（キーあり=api_key、なし=entra）。
# AUTH_MODE=
...
.env.example                # 環境設定テンプレート（トンネル名、ドメイン、アプリ ID）
...
3. **`.env` を設定してマニフェストを生成**
   ```bash
   cp .env.example .env
   # .env を編集: TUNNEL_NAME, TEAMS_DOMAIN, TEAMS_APP_ID を設定
   python teams/generate_package.py
   ```
   `.env` を読み込み、テンプレートの `{{APP_ID}}` と `{{DOMAIN}}` を置換し、アイコン生成＆ zip パッケージを作成します。

4. **Teams にサイドロード**

   前のステップで `teams/realtimeqa-teams.zip` が自動生成されます。
...
各サービスが **Entra ID と API キーの両方**に対応し、サービスごとに独立して選べるため、各ユーザーが手元の資格情報をそのまま使えます：

- **Foundry**（要約/QA）: キーモードは `AZURE_OPENAI_API_KEY` を設定、空にすると Entra ID。
- **Speech**（文字起こし）: キーモードは `AZURE_SPEECH_KEY` を設定（バックエンドが区域 `issueToken` エンドポイントで短時間トークンに交換し、キーはブラウザに渡りません）、または `AZURE_SPEECH_RESOURCE_ID` で Entra ID。

モードはサービスごとに**自動判定**（キーあり→api-key、なければ Entra）されます。`AUTH_MODE=api_key|entra` で全体を固定、またはサービス個別に `AZURE_OPENAI_AUTH_MODE` / `AZURE_SPEECH_AUTH_MODE` で上書きできます。既存のキーなし設定はそのまま動作します。
