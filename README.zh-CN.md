...
# Azure Speech — 二选一：填密钥（api-key 模式）或填资源 ID（Entra 模式）
AZURE_SPEECH_REGION=eastus2
# api-key 模式：填资源密钥（后端换成短时 token，密钥不会下发到浏览器）
AZURE_SPEECH_KEY=
# Entra 模式：改填 ARM 资源 ID
AZURE_SPEECH_RESOURCE_ID=/subscriptions/<sub-id>/resourceGroups/<rg>/providers/Microsoft.CognitiveServices/accounts/<name>
# 可选：强制认证模式（api_key | entra）。默认自动判断（有密钥=api_key，否则=entra）。
# AUTH_MODE=
...
| 方式 | 命令 |
|------|------|
| **Azure CLI**（推荐） | **Windows:** `winget install Microsoft.AzureCLI` / **macOS:** `brew install azure-cli` → 打开新终端 → `az login` |
...
.env.example                # 环境配置模板（隧道名、域名、应用 ID）
start-dev.ps1               # 并行启动后端 + 前端 (Windows)
start-dev.sh                # 并行启动后端 + 前端 (macOS / Linux)
start-tunnel.ps1            # 启动 Dev Tunnel（用于 Teams 测试） (Windows)
start-tunnel.sh             # 启动 Dev Tunnel (macOS / Linux)
...
3. **配置 `.env` 并生成清单**
   ```bash
   cp .env.example .env
   # 编辑 .env：设置 TUNNEL_NAME、TEAMS_DOMAIN、TEAMS_APP_ID
   python teams/generate_package.py
   ```
   脚本会读取 `.env`，替换模板中的 `{{APP_ID}}` 和 `{{DOMAIN}}`，生成图标并打包 zip。

4. **旁加载到 Teams**

   上一步已自动生成 `teams/realtimeqa-teams.zip`。
...
每个服务都同时支持 **Entra ID 和 API 密钥**，可按服务独立选择，让每个用户用自己手头的凭证：

- **Foundry**（摘要/问答）：填 `AZURE_OPENAI_API_KEY` 走密钥模式，留空则用 Entra ID。
- **Speech**（转录）：填 `AZURE_SPEECH_KEY` 走密钥模式（后端通过区域 `issueToken` 端点换成短时 token，密钥不会下发到浏览器），或填 `AZURE_SPEECH_RESOURCE_ID` 用 Entra ID。

模式按服务**自动判断**（有密钥→api-key，否则 Entra），也可用 `AUTH_MODE=api_key|entra` 全局强制，或用 `AZURE_OPENAI_AUTH_MODE` / `AZURE_SPEECH_AUTH_MODE` 按服务覆盖。现有无密钥配置保持不变。

## 开发脚本（v3.1）

| 脚本 | 说明 |
|------|------|
| `start-dev.ps1` | 并行启动后端 (FastAPI:8000) + 前端 (Vite:5173) (Windows) |
| `start-tunnel.ps1` | 启动 Dev Tunnel（用于 Teams 测试、HTTPS 暴露） (Windows) |
| `start-dev.sh` | 并行启动后端 + 前端 (macOS / Linux) |
| `start-tunnel.sh` | 启动 Dev Tunnel (macOS / Linux) |
